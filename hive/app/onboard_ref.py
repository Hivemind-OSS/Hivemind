...
ONBOARDING_PROCEDURE: str = (
    ...
    + _RUNTIME_TABLE + "\n"
    "1) Write the served HIVEMIND-RULES block into that file BETWEEN its markers, CREATING the "
    "file if it does not exist (a fresh clone ships no rules file). If it exists without a "
    "HIVEMIND-RULES:START..END block, APPEND the block; if such a block is already present, "
    "REPLACE it in place. In every case: one block — no duplicate, no clobber of the surrounding "
    "content.\n"
