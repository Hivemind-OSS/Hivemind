...
SERVER_INSTRUCTIONS = (
    ...
    "few memories that truly can't wait; let everything else ride capture->demand. Before you "
    "write, hive_recall the topic and act on what already exists: don't duplicate a memory that "
    "already says it; if one is now wrong, correct it in place with hive_write(replaces=<episode_id>) "
    "so the old one retires; if your fact contradicts an existing memory and you can't tell which is "
    "right, resolve that first (see RESOLVE) rather than adding a rival.\n"
    "• RESOLVE duplicates, contradictions & wrong memories — a recall may carry a conflicts list (near-duplicate "
    "or opposing memories, by id), and hive_health(include_conflicts=true) is the full worklist. "
    "When you see one — or notice a recalled memory contradicts another memory or your task "
    "context — ALERT your human and request a resolution; never retire a memory on your own "
    "judgment. Name the loser for a clear redundancy; for a symmetric contradiction with no "
    "obvious loser, ask your human which memory wins first. Then retire it with "
    "hive_supersede(loser, winner, approved_by=...) — human-vouched, the loser is deprecated "
    "immediately. For a lone recalled memory that is simply WRONG — incorrect or misleading, with "
    "no corrected version to write and no other memory to name as a winner — request "
    "hive_prune(episode_id, approved_by=...), a human-vouched bare retirement (use it, not "
    "supersede, when nothing replaces the bad memory); if you are unsure or only saw it HURT your "
    "task, hive_outcome(hurt=[episode_id]) records that evidence — it retires nothing — for a "
    "human to review. For a semantic conflict the automatic scan can't see, "
