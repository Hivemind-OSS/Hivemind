...
def _row_repos(r: Mapping[str, Any]) -> set[str]:
...
def _cos(a: "np.ndarray", b: "np.ndarray") -> float:
    ...
    if (
        na <= 0.0
        or nb <= 0.0
        or not (np.all(np.isfinite(a)) and np.all(np.isfinite(b)))
    ):
...
def _cluster(
    rows: Sequence[Mapping[str, Any]], *, tau: float
    ...
    for r in rows:
        ...
        for c in clusters:
            ...
            clusters.append(
                {
                    "_vec": vec,
                    "representative_query": r["query_text"],
                    "miss_count": 1,
...
def cluster_misses(
    rows: Sequence[Mapping[str, Any]], *, tau: float, top_n: int = 10
) -> list[dict[str, Any]]:
    ...
    return [
        {
            **{k: v for k, v in c.items() if k not in ("_vec", "_repos")},
            "repos": sorted(c["_repos"]),
        }
        for c in out[:top_n]
    ]
