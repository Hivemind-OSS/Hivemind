"""Validate an extraction dict's shape before graph assembly.

A lightweight schema check on the ``{nodes, edges}`` payload ``build_from_json``
consumes: every node carries the required identity fields, every edge names a
known confidence and endpoints that resolve to a node. Dangling edges to
external/stdlib symbols are *expected* (they are filtered at assembly), so the
assembler treats "does not match any node id" as a non-error.
"""
...
def validate_extraction(data: dict[str, Any]) -> list[str]:
    ...
    # hashable ids land here; a non-hashable id (e.g. a list) is reported as an
    # error rather than crashing the validator on set construction.
    ...
    # Edges - accept "links" (NetworkX <= 3.1) as a fallback for "edges"
