"""Bespoke SQL extractor (CREATE/ALTER/views/foreign keys), not config-driven."""
...
from .support import _file_stem, _make_id
...
def extract_sql(path: Path, content: str | bytes | None = None) -> dict[str, Any]:
    ...
    def _read(n: Node) -> str:
        return source[n.start_byte : n.end_byte].decode("utf-8", errors="replace")

    def _obj_name(n: Node) -> str | None:
        for c in n.children:
    ...
    def _ref_target(name: str) -> str:
        """Resolve a references/reads_from/triggers TARGET to a node id.

        A table defined in THIS file resolves to its real node. A cross-file
        candidate is emitted as a SOURCELESS stub (empty source_file) keyed by the
        bare name, so the corpus-level ``_rewire_unique_stub_nodes`` pass can collapse
        it onto the real definition when the name resolves unambiguously (exactly one
        table with that label anywhere in the corpus). A sourced stub would let
        ``_disambiguate_colliding_node_ids`` bake this file's path into the id and
        block the rewire (the phantom-duplicate node), so the stub carries no source.
        """
        same_file = table_nids.get(name.lower())
        if same_file:
            return same_file
        stub_id = _make_id(name)
    ...
    def _walk_from_refs(node: Node, caller_nid: str, line: int) -> None:
