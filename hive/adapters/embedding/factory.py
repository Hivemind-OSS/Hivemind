...
load; ``load()`` later asserts the real model agrees. ``model`` is an injection seam for tests
(pass an already-loaded SentenceTransformer / a double) so the provider can be exercised
...
def build_provider(cfg: "Config", *, model=None) -> LocalSTEmbedder:
    ...
    return LocalSTEmbedder(
        model_name=cfg.embedding.model, d=native_dim_for(cfg.embedding.model), model=model)
