...
SERVICE = (
    "hive-server"  # single source of the compose service name (mirrors compose.yaml)
)
...
_DOTENV = (
    ".env"  # repo-root operator config; the SAME file compose auto-loads (cwd-relative)
)


def _load_dotenv(path: str, base: Mapping[str, str]) -> dict[str, str]:
...
class StatusSnapshot:
    ...
    tunnel_url: Optional[str] = (
        None  # the public /mcp URL when the tunnel is up + a domain is set
    )
    seats: Optional[int] = (
...
def _wait_healthy(run: Run, env: Mapping[str, str]) -> int:
    ...
    timeout_s = int(
        (env.get("HIVE_HEALTH_TIMEOUT") or "").strip() or DEFAULT_HEALTH_TIMEOUT_S
    )
...
def _up(
    ...
    *,
    ...
    if args.tunnel:
        ...
        if missing:
            print(
                f"hive: --tunnel requires {' + '.join(missing)} "
...
_DEFAULT_RESET_OUT = (
    "hive-backups"  # host dir (cwd-relative) for reset's + upgrade's snapshot
)


def _copy_snapshot_into_volume(
    run: Run, env: Mapping[str, str], src: str
) -> subprocess.CompletedProcess[str]:
    ...
    return run(
        [
            ...
            "hive-data:/data",
            "-v",
            f"{os.path.dirname(src)}:/in",
            IMAGE,
            "-c",
            f"cp /in/{name} /data/shared.db && "
            "rm -f /data/shared.db-wal /data/shared.db-shm",
        ],
        env,
    )


def _snapshot_to_host(
    run: Run, env: Mapping[str, str], abs_out: str
    ...
    snap = run(
        [
            ...
            f"{abs_out}:/out",
            "-e",
            "HIVE_RETENTION__BACKUP_DIR=/out",
            IMAGE,
            "-m",
...
def _reset(
    ...
    *,
    ...
    if not args.yes:
        answer = ask(
            f"hive reset DESTROYS the data volume (a snapshot is saved to {abs_out} "
    ...
    if snap.returncode != 0:
        ...
        print(
            "hive: pre-reset snapshot FAILED — nothing destroyed "
            "(run `hive up` if you meant to start a fresh store)",
            file=sys.stderr,
        )
        return EX_UNAVAILABLE
    ...
    if run(_compose("down", "-v"), env, capture=False).returncode != 0:
        return EX_UNAVAILABLE
    if (
        run(_compose("up", "-d", "--build", SERVICE), env, capture=False).returncode
        != 0
    ):
        return EX_UNAVAILABLE
    rc = _wait_healthy(run, env)
    print(
        f"hive: reset complete — prior store saved to {host_dest}\n"
        f"      roll back with: hive restore {host_dest}",
        file=sys.stderr,
    )
    return rc
...
def _restore(
    ...
    *,
    ...
    if not args.yes:
        answer = ask(
            f"hive restore OVERWRITES the live store with {src}. Type 'restore' to confirm: "
        )
    ...
    if cp.returncode != 0:
        ...
        print(
            "hive: restore copy FAILED — the daemon is stopped; run `hive up`",
            file=sys.stderr,
        )
        return EX_UNAVAILABLE
    if (
...
_DEFAULT_UPGRADE_REF = (
    "release"  # the maintainer's vetted-release pin; roll back with an older tag
)
...
def _rollback(run: Run, env: Mapping[str, str], *, prev: str, snap: str) -> int:
    ...
    def _manual(reason: str) -> int:
        print(
            f"hive: AUTOMATIC ROLLBACK FAILED ({reason}) — recover manually:\n"
            f"      git checkout {prev}\n"
            f"      hive restore {snap}",
            file=sys.stderr,
        )
    ...
    if (
        run(_compose("down"), env, capture=False).returncode != 0
    ):  # release WAL locks before the copy
    ...
    print(
        f"hive: rolled back to {prev[:12] or 'HEAD'} — store restored from {snap}",
        file=sys.stderr,
    )
    return EX_UNAVAILABLE
...
def _upgrade(
    ...
    *,
    ...
    if (run(["git", "status", "--porcelain"], env).stdout or "").strip():
        print(
            "hive: working tree is dirty — commit or stash before `hive upgrade` "
            "(nothing changed)",
            file=sys.stderr,
        )
        return EX_USAGE
    if run(["git", "fetch", "--tags"], env).returncode != 0:
        print(
            "hive: `git fetch` failed — cannot reach the remote (nothing changed)",
            file=sys.stderr,
        )
        return EX_UNAVAILABLE
    if run(["git", "rev-parse", "--verify", "--quiet", ref], env).returncode != 0:
        print(
            f"hive: ref {ref!r} not found after fetch (nothing changed)",
            file=sys.stderr,
        )
        return EX_USAGE
    ...
    if not args.yes:
        answer = ask(
            f"hive upgrade snapshots the store, checks out {ref!r}, rebuilds, and "
    ...
    if run(["git", "checkout", ref], env).returncode != 0:
        print(
            f"hive: `git checkout {ref}` failed — server unchanged; snapshot at {host_dest}",
            file=sys.stderr,
        )
        return EX_UNAVAILABLE
    healthy = (
    ...
    if healthy:
        print(
            f"hive: upgrade complete — now at {ref} (snapshot saved to {host_dest})",
            file=sys.stderr,
        )
        return EX_OK
    print(
        f"hive: {ref} did not come up healthy — rolling back to {prev[:12] or 'HEAD'}",
        file=sys.stderr,
    )
...
def _probe_status(run: Run, env: Mapping[str, str]) -> StatusSnapshot:
    ...
    seats = (
        len([ln for ln in (tok.stdout or "").splitlines() if ln.strip()])
        if tok.returncode == 0
        else None
    )
    return StatusSnapshot(
        server="up",
        healthy=hc.returncode == 0,
        tunnel_on=tunnel_on,
...
def _exec_authctl(
    run: Run, env: Mapping[str, str], *args: str
) -> subprocess.CompletedProcess[str]:
...
def _token(
    ...
    *,
    ...
    print(
        f"hive: token for seat {args.seat!r} shown once above — hand it over via a "
        f"secret manager; {_SEAT_HINT}",
        file=sys.stderr,
    )
    return EX_OK
...
def _exec_backup(run: Run, env: Mapping[str, str]) -> subprocess.CompletedProcess[str]:
    ...
    return run(
        _compose("exec", "-T", SERVICE, "python", "-m", "hive.tools.backupctl"), env
    )
...
def _ingest(
    ...
    *,
    ...
    try:
        ...
        return EX_USAGE
    flags = (
        ["--post-merge", "--verdict", args.verdict, "--signal", args.signal]
        if args.post_merge
        else []
    )
    child = run(
        _compose("exec", "-T", SERVICE, *CENSUSCTL, "ingest", "-", *flags),
        env,
        input=receipt_text,
    )
    sys.stderr.write(child.stderr or "")
...
def _ui(
    ...
    *,
    ...
    return ui.serve_ui(
        host=args.host, port=args.port, run=run, env=env, open_browser=not args.no_open
    )
...
def main(
    ...
    *,
    ...
    p_up = sub.add_parser(
        "up", help="build + start the daemon; bounded wait until healthy"
    )
    p_up.add_argument(
        "--tunnel",
        action="store_true",
    ...
    p_reset = sub.add_parser(
        "reset",
        help="snapshot the store out of the volume, then destroy + recreate it empty "
    ...
    p_restore = sub.add_parser(
        "restore",
        help="replace the live store with a snapshot .db (inverse of reset; asks to confirm)",
    )
    p_restore.add_argument(
        "file", help="path to a snapshot .db (e.g. ./hive-backups/hive-<stamp>.db)"
    )
    p_restore.add_argument(
    ...
    p_upgrade = sub.add_parser(
        "upgrade",
        help="move the server to a vetted release ref: snapshot → checkout → rebuild → "
        "health-gate → auto-rollback on failure (backup-gated)",
    )
    p_upgrade.add_argument(
        "--ref",
        default=_DEFAULT_UPGRADE_REF,
    ...
    sub.add_parser(
        "backup",
        help="snapshot the store now (manual; keeps the backup_keep most-recent you take)",
    )
    sub.add_parser("status", help="server health, tunnel state + URL, seat count")
    p_token = sub.add_parser(
        "token", help="mint a per-seat token (printed ONCE to stdout)"
    )
    p_token.add_argument(
        "seat", help="agent-seat identity, e.g. alice-laptop (one per seat)"
    )
    p_revoke = sub.add_parser(
        "revoke", help="revoke a seat's token (next request → 401)"
    )
    ...
    p_repo = sub.add_parser(
        "repo",
        help="manage the synced-repo registry (add/remove; list with `hive repos`)",
    )
    ...
    sub.add_parser(
        "connect", help="print the teammate's `claude mcp add` line (transport only)"
    )
    p_ui = sub.add_parser(
    ...
    p_ui.add_argument(
        "--port", type=int, default=4173, help="loopback port (default: 4173)"
    )
    p_ui.add_argument(
        "--no-open",
        action="store_true",
    ...
    p_ingest = sub.add_parser(
        "ingest",
        help="feed an unsigned census receipt's outcome into the evidence ledger",
    )
    p_ingest.add_argument(
        "receipt", help="path to the receipt JSON (piped to the in-container censusctl)"
    )
    p_ingest.add_argument(
        "--post-merge",
        action="store_true",
    ...
    p_ingest.add_argument(
        "--signal",
        choices=("randomized", "canary", "none"),
        default="none",
        help="post-merge evidence signal — machine-checked requires randomized/canary",
    )
...
if (
    __name__ == "__main__"
):  # pragma: no cover — module entry (`python -m hive.tools.cli`)
