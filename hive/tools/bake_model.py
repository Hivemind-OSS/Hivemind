...
def bake(
    model: str, dest: str, *, loader: Optional[Callable[[str, str], None]] = None
) -> int:
    ...
    try:
        ...
        _log.error(
            "bake.failed model=%s dest=%s kind=%s", model, dest, type(exc).__name__
        )
        return FAILED
...
def main(
    ...
    *,
    loader: Optional[Callable[[str, str], None]] = None,
) -> int:
    parser = argparse.ArgumentParser(
        prog="hive.tools.bake_model",
        description="Bake ST weights offline into an image layer.",
    )
    parser.add_argument(
        "--model", required=True, help="HF model id, e.g. Qwen/Qwen3-Embedding-0.6B"
    )
    parser.add_argument(
        "--dest", required=True, help="cache dir (must match runtime HF_HOME)"
    )
