...
def main(
    ...
    *,
    ...
    p_ingest = sub.add_parser(
        "ingest",
        help="decode + validate a DSSE receipt, join touched symbols "
    ...
    if args.receipt == "-":
        ...
        try:
            ...
            print(
                f"censusctl: cannot read receipt {args.receipt!r}: {error}",
                file=sys.stderr,
            )
            return EX_DATAERR
    try:
    ...
    try:
        store = SqliteEpisodeStore(
            connect_fn(db_path), index=None
        )  # torch/embedder-free
        service = ChangeEvidenceService(
            reader=store, appender=store, now=now, ranges=store
        )
        report = service.ingest(
            envelope,
            phase="post_merge" if args.post_merge else "pre_merge",
    ...
    print(
        f"censusctl: matched={report.matched} inserted={len(report.inserted)} "
...
if (
    __name__ == "__main__"
):  # pragma: no cover — module entry (`python -m hive.tools.censusctl`)
