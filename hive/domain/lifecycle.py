...
class LifecycleService:
    ...
    def sweep(self, now: Optional[int] = None) -> dict:
        """Materialize lazy decay (boot + post-capture piggyback). Inert when
        disabled; a sweep fault is logged, never raised."""
        if not self.enabled:
            return {}
        try:
            t = self._now() if now is None else int(now)
            return self._store.sweep_decayed(now=t, q_ttl_s=self._q_ttl,
                                             p_ttl_s=self._p_ttl)
