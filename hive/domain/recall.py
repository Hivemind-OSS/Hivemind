...
from hive.domain.lifecycle import is_servable, scope_matches
from hive.domain.models import (
    CONFIDENT, Episode, RecallHit, RecallResult, Scored,
)
from hive.domain.ports import (
    EmbeddingProvider, EpisodeReader, ExposureLedger, RepoScopeReader,
    VectorIndex,
)
...
def _scope_filter(
    ...
    for eid, sim in candidates:
        ...
        if prefix and anchor_pairs and not any(
                anchor.startswith(prefix) for _repo, anchor in anchor_pairs):
...
def select_served(items: Sequence[tuple[Scored, Episode]], *, dup_tau: float,
    ...
    citems = [ConflictItem(episode_id=ep.id, vector=ep.value, polarity=ep.polarity,
                           anchor=_conflict_anchor(ep), ts=ep.ts, trust=ep.trust)
              for _s, ep in items]
...
class RecallPipeline:
    ...
    def recall(self, query: str, *, agent_id: str,
               scope: Optional[RecallScope] = None) -> RecallResult:
        ...
        try:
            ...
            hits = tuple(
                RecallHit(s.episode_id, ep.text, s.sim, trust=ep.trust, ts=ep.ts,
                          polarity=ep.polarity, kind=ep.kind, repos=ep.repos,
                          anchors=ep.anchors, meta=ep.meta)
    ...
    def _note_non_answer(self, query: str, vector, agent_id: str,
                         miss_type: str,
                         scope: Optional[RecallScope] = None) -> None:
        ...
        try:
            ...
            if scope is not None and scope.repos:
                repos_json = json.dumps(sorted(scope.repo_names),
                                        separators=(",", ":"))
