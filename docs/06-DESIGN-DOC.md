...
**Cross-cutting locked facts:** hexagonal; single Docker service; one SQLite-WAL volume; **exactly 8 MCP tools**; geometry `bge-small(384)→PCA→d=256` dense cosine; exhaustive index authoritative; normalized-entropy abstention `H/ln(N_eff)>0.5`.
