...
| **D10** (2026-06-11) | Fleet convergence (CONVERGENCE-PLAN CV1–CV5) | **Reach + the completed mechanical ladder + mechanical self-correction + the convergence KPI, all default-preserving**: codex tier-1 profile + vendorable stdlib `hive/client.py` (HiveError-or-payload, fence-tested against the real run_http stack) + the seat-token contract (one token per seat — identity is the promotion fuel); `SurvivalRule` (≥2 non-writer identities / ≥5 exposures / ≥14d span, sweep-time, same `established` state + `establish` audit row); `autonomy.solo_mode` swaps demand's diversity clause for ELAPSED-span (≥24h first-to-last; established stays human-only at solo scale; `solo_hint` self-describes the stall); serve-time `shadow_filter` (trust-rank → newer-ts → lower-id, at RESOLVE before exposure) behind `recall.shadow` OFF + a golden; contested-memory report + `hive_health(include_trends)` demand-entropy KPI; dev-time `gate_eval` recommending only on paired-bootstrap `lo>0`. | *Bare diversity waiver / calendar-day buckets / client-claimed sub-identities* (D-C1); *a fourth `established_survival` trust state* (D-C2: splits the serving predicate for an audit-level distinction); *filter-after-exposure & newest-wins shadowing* (D-C3: liveness resurrection; a fresh capture must not hide a vouched row); *widening ExposureLedger into a lifecycle Protocol* (D-C4: duck-typed precedent, real-adapter behavior tests); *server-side LLM / schedulers / autonomous retirement of established* (standing refusals). CV6 (host-side `hive credit` outcome scan → keystone) **pending ADMIN-CLI** — boundary locked: the server never reads repos. |

**Cross-cutting locked facts:** hexagonal; single Docker service; one SQLite-WAL volume; **exactly 6 MCP tools** (v3 client-gating dropped the queue verbs; the lifecycle build added `hive_capture`); geometry `bge-small(384)→PCA→d=256` dense cosine; exhaustive index authoritative; normalized-entropy abstention `H/ln(N_eff)>0.5`; serving = `lifecycle.is_servable` (established, or fresh provisional, labeled); hybrid lexical channel (FTS5+RRF, confident-branch-only) **implemented, default-off** pending `channel_eval` CI evidence; the mechanical trust ladder is COMPLETE (demand → provisional → survival-spread → established; retirement of established stays human-only); serve-time shadowing **implemented, default-off** behind a golden; **one token per agent seat** is the operational contract (solo escape hatch: `autonomy.solo_mode`, elapsed-span demand).
...
- **Serve-time shadowing implemented, gated OFF** (2026-06-11, D10) — `recall.shadow=false` (tier C) behind a golden byte-stability test; flip-on evidence is an `eval_membrane` replay showing only intended near-dup suppression. **Survival-established wrong content is TTL-immune** (D-C2, accepted): audit row + contested report + one-write supersession bound it. **CV6 outcome credit (`hive credit` host-side scan) is pending ADMIN-CLI** — until it lands, trends measure coverage, not outcomes; `task_outcomes`/keystone stay dormant-but-fed-ready.
