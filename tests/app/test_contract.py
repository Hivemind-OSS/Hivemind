...
def test_instructions_teach_the_v3_loop():
    ...
    assert "hive_outcome" in s and "evidence only" in low  # outcome is evidence, not control
    ...
    for verb in ("hive_recall", "hive_write", "hive_capture", "hive_supersede",
...
def test_remediation_notice_is_agent_adjudicated():
    ...
    for token in ("hive_outcome", "hurt=[", "hive_supersede", "hive_write(replaces=",
                  "hive_prune", "BAD_VS_STALE"):
