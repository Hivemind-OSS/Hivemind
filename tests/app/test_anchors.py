...
def test_multi_anchor_keeps_input_order():
    out = normalize_anchors(
        [{"repo": "beta", "anchor": "b.py::g"},
         {"repo": "alpha", "anchor": "a.py::f"}],
        known_repos=KNOWN)
    assert [(a.repo, a.anchor) for a in out] == [
        ("beta", "b.py::g"), ("alpha", "a.py::f")]


def test_injected_read_accepts_a_callable():
    out = normalize_anchors(
        [{"repo": "alpha", "anchor": "a.py"}],
...
def test_anchor_is_free_text():
    ...
    out = normalize_anchors(
        [{"repo": "alpha", "anchor": weird}], known_repos=KNOWN)
...
def test_unknown_repo_refuses_and_names_the_repo():
    with pytest.raises(BadAnchors, match="unknown repo 'ghost'"):
        normalize_anchors(
            [{"repo": "ghost", "anchor": "x.py::f"}], known_repos=KNOWN)
...
def test_hostile_shapes_refuse_never_crash():
    ...
    hostile = [object(), {1: 2}, [{b"repo": b"x"}], [{"repo": None, "anchor": None}],
               [None], [True], 0.5, {"a"}]
    for raw in hostile:
...
def test_entry_cap_refuses():
    raw = [{"repo": "alpha", "anchor": f"f{i}.py"} for i in range(SCOPE_MAX_ENTRIES + 1)]
