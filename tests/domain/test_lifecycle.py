...
def test_decayed_quarantine_activity_refreshes():
    ...
    assert (
        _dec(trust=QUARANTINED, created_ts=NOW - 10 * Q_TTL, last_active_ts=NOW - 1)
        is False
    )
...
def test_decayed_provisional_boundary():
    ...
    assert (
        _dec(trust=PROVISIONAL, last_active_ts=NOW - P_TTL - 1, created_ts=NOW) is True
    )  # a recent created_ts does NOT revive it
...
def test_sweep_is_decay_only():
    ...
    assert (
        "survival_rule" not in inspect.signature(LifecycleService.__init__).parameters
    )
