...
from hive.domain.consensus import (
    SuspectConsensusItem, SuspectConsensusNote, detect_suspect_consensus,
)


def _item(eid, *, n_eff, k, rho_bar=0.9, has_settled_win=False, anchor="f.py"):
    return SuspectConsensusItem(episode_id=eid, rho_bar=rho_bar, n_eff=n_eff, k=k,
                                has_settled_win=has_settled_win, anchor=anchor)


# ── SuspectConsensusNote invariants (illegal states unconstructable) ───────────
def test_note_rejects_out_of_range_rho_bar():
    with pytest.raises(ValueError):
        SuspectConsensusNote(episode_id=1, rho_bar=1.5, n_eff=1.0, k=3,
                             martingale_warning=True)
    with pytest.raises(ValueError):
        SuspectConsensusNote(episode_id=1, rho_bar=float("nan"), n_eff=1.0, k=3,
                             martingale_warning=True)


def test_note_rejects_negative_n_eff_and_k():
    with pytest.raises(ValueError):
        SuspectConsensusNote(episode_id=1, rho_bar=0.5, n_eff=-0.1, k=3,
                             martingale_warning=True)
    with pytest.raises(ValueError):
        SuspectConsensusNote(episode_id=1, rho_bar=0.5, n_eff=float("inf"), k=3,
                             martingale_warning=True)
    with pytest.raises(ValueError):
        SuspectConsensusNote(episode_id=1, rho_bar=0.5, n_eff=1.0, k=-1,
                             martingale_warning=True)


def test_note_is_ids_and_numbers_only():
    # the carrier shape: episode_id + numbers + the warning bool — no text, no anchor.
    note = SuspectConsensusNote(episode_id=7, rho_bar=0.9, n_eff=1.0, k=4,
                                martingale_warning=True)
...
def test_sort_by_frac_then_episode_id():
    ...
    items = [_item(30, n_eff=1.0, k=4),     # 0.25
             _item(10, n_eff=0.4, k=4),     # 0.10
             _item(20, n_eff=0.4, k=4)]     # 0.10
    notes = detect_suspect_consensus(items, n_eff_frac_max=0.5)
    assert [n.episode_id for n in notes] == [10, 20, 30]   # (0.10,10),(0.10,20),(0.25,30)
...
def test_degenerate_guards_return_empty():
    assert detect_suspect_consensus([_item(1, n_eff=1.0, k=4)], n_eff_frac_max=0.5,
                                    top_n=0) == ()
    assert detect_suspect_consensus([_item(1, n_eff=1.0, k=4)],
                                    n_eff_frac_max=float("nan")) == ()
...
def test_martingale_warning_thin_and_no_settled_win_is_true():
    notes = detect_suspect_consensus([_item(40, n_eff=1.0, k=4, has_settled_win=False)],
                                     n_eff_frac_max=0.5)
...
def test_martingale_warning_thin_but_settled_win_is_false():
    ...
    notes = detect_suspect_consensus([_item(41, n_eff=1.0, k=4, has_settled_win=True)],
                                     n_eff_frac_max=0.5)
