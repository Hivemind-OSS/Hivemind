...
def test_conflict_is_symmetric_winner_dropped():
    ...
    svc, _ = _svc(flags=flags)
    svc.flag(
        kind="conflict", a_id=10, b_id=20, winner_id=20, resolution="", proposed_by="a"
    )
...
def test_flag_never_retires_episode_state_unchanged():
    ...
    a, _ = store.stage(text="A: deploy with make", weight=1.0, proposed_by="w")
    store.complete(a, vec, expected_version=0, trust="established", last_active_ts=10)
    b, _ = store.stage(text="B: deploy with ship.sh", weight=1.0, proposed_by="w")
    store.complete(b, vec, expected_version=0, trust="established", last_active_ts=10)
