...
from hive.domain.lifecycle import (
    ESTABLISHED, PROVISIONAL, QUARANTINED, DemandRule, LifecycleService, MissRow,
    PromotionDecision, _demand_independence, scope_matches, verified_win_decision,
)
...
def _miss(agent: str, *, cos: float = 0.9, ts: int = NOW - 10,
          repos: tuple[str, ...] = ()) -> MissRow:
...
def _decide(rule: DemandRule, misses, *, writer: str = "writer-1",
            ...
            candidate_repos: tuple[str, ...] = ()) -> PromotionDecision:
...
def test_other_repo_scoped_misses_never_count():
    ...
    misses = [_miss("other-1", repos=("beta",)), _miss("other-2", repos=("beta",)),
...
def test_intersecting_scopes_count():
    misses = [_miss("other-1", repos=("alpha", "gamma")),
              _miss("other-2", repos=("alpha",)), _miss("other-3", repos=("alpha",))]
...
def test_scope_clause_runs_before_the_cosine_clause():
    ...
    poisoned = MissRow(vector=np.array([np.nan, 0, 0, 0], dtype=np.float32),
                       agent_id="other-9", ts=NOW - 10, repos=("beta",))
    good = [_miss("writer-1"), _miss("other-1"), _miss("other-2")]
    d = _rule().decide(candidate_vec=CAND, candidate_writer="writer-1",
                       misses=[poisoned, *good], competitor_top_sim=-1.0,
...
class _LifeStore:
    ...
    def __init__(self, candidates=(), misses=(), index: FakeIndex | None = None,
                 repos_by_eid=None, servable_scopes_map=None):
        self.candidates = list(candidates)   # (eid, vec, proposed_by, ts, last_active)
        self.misses = list(misses)
        self.index = index
        self.repos_by_eid = {int(k): tuple(v)
                             for k, v in dict(repos_by_eid or {}).items()}
        # eid → repo scope of SERVABLE rows (the partition-aware competitor read)
        self.servable = {int(k): tuple(v)
                         for k, v in dict(servable_scopes_map or {}).items()}
    ...
    def get_episode(self, episode_id):
        ...
        if eid not in self._vec_by_eid and eid not in self.repos_by_eid:
            return None
        return make_episode(eid, f"candidate-{eid}", status="approved",
                            trust=QUARANTINED, repos=self.repos_by_eid.get(eid, ()))
...
def test_on_capture_scope_matched_demand_promotes_scoped_candidate():
    st = _LifeStore(candidates=[(7, CAND, "writer-1", NOW - 5, NOW - 5)],
                    misses=[_miss("other-1", repos=("alpha",)),
                            _miss("other-2", repos=("alpha",)),
                            _miss("other-3", repos=("alpha",))],
                    index=FakeIndex(), repos_by_eid={7: ("alpha",)})
    d = _service(st).on_capture(7)
    assert d is not None and d.promote is True
    assert st.trust[7] == (PROVISIONAL, NOW)
...
def test_on_capture_other_repo_demand_never_promotes():
    ...
    st = _LifeStore(candidates=[(7, CAND, "writer-1", NOW - 5, NOW - 5)],
                    misses=[_miss("other-1", repos=("beta",)),
...
def _scoped_demand(repo: str) -> list[MissRow]:
    return [_miss("other-1", repos=(repo,)), _miss("other-2", repos=(repo,)),
...
def test_competitor_in_another_repo_does_not_veto():
    ...
    st = _LifeStore(candidates=[(7, CAND, "writer-1", NOW - 5, NOW - 5)],
                    misses=_scoped_demand("alpha"), index=idx,
...
def test_unresolvable_competitor_scope_is_treated_general_and_vetoes():
    ...
    st = _LifeStore(candidates=[(7, CAND, "writer-1", NOW - 5, NOW - 5)],
                    misses=_scoped_demand("alpha"), index=idx,
                    repos_by_eid={7: ("alpha",)}, servable_scopes_map={})
    out = _service(st).on_miss(_at_cos(0.95), agent_id="other-9")
    assert dict(out)[7].reason == "competitor_answers"
...
def test_writer_only_demand_over_any_span_never_promotes():
    ...
    d = _rule().decide(candidate_vec=CAND, candidate_writer="writer-1",
                       misses=spread, competitor_top_sim=-1.0, candidate_repos=())
    assert d.promote is False and d.reason == "self_demand"
...
def _est_service(store, *, reader, enabled: bool = True):
    return LifecycleService(
        store=store, index=FakeIndex(), rule=_rule(), now=lambda: NOW,
        demand_window_s=WINDOW_S, quarantine_ttl_s=Q_TTL, provisional_ttl_s=P_TTL,
        enabled=enabled, verified_reader=reader)
