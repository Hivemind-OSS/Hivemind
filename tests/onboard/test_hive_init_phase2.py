...
table); an unwatched repo still links (watch_warning is non-blocking).
"""
...
def _planner(store, *, trailer="Hive-Trace", watch=()):
    return InstallPlanner(store, stamp_trailer=trailer, watch_repos=watch)
...
def test_phase2_good_hash_links(tmp_path):
    store = _store()
    p = _planner(store, watch=(str(tmp_path),))
...
def test_phase2_links_despite_unwatched_repo(tmp_path):
    store = _store()
    p = _planner(store, watch=("/other/repo",))           # repo is NOT watched
    plan = p.plan(str(tmp_path), "generic")
    assert plan.watch_warning is not None                 # warned in phase-1
    res = p.confirm(str(tmp_path), plan.expected_confirm_hash)
    assert res["linked"] is True                          # but still links (non-blocking)
