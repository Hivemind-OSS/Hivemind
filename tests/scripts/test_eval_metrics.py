"""Locked assertions for the ``scripts.eval_metrics`` scorers: the pure IR metrics
(``recall_at_k``, ``mrr``) and the significance machinery (``abstention_auroc``,
...
def test_auroc_nan_score_raises():
    # a NaN score breaks the sort + tie-detection and would return a clean WRONG
    # float (brute-force AUROC differs); NaN is undefined ⇒ refuse, never mask.
