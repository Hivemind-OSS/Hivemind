...
def _serve(spy, verify, **kw):
    ...
    httpd = ThreadingHTTPServer(("127.0.0.1", 0),
                                _build_handler(spy, verify, threading.Lock(), **kw))
...
def test_valid_signature_is_204_sets_nudge_pre_bearer_and_never_parses_payload():
    ...
    try:
        status, resp, _ = _request(f"{base}/census-webhook", body=body,
...
def test_webhook_bad_signature_is_401_and_nudge_never_fires():
    ...
    ev = threading.Event()
    spy = _SpyServer()
    base, stop = _serve(spy, lambda tok: None, webhook_secret="s3cret", webhook_nudge=ev.set)
    body = b"payload-bytes"
    good = _sig("s3cret", body)
    tampered = good[:-1] + ("0" if good[-1] != "0" else "1")
    try:
        for sig in (_sig("wrong-secret", body),            # signed with the wrong secret
            ...
            status, resp, _ = _request(f"{base}/census-webhook", body=body, headers=hdrs)
            assert status == 401
...
def test_unset_secret_keeps_census_webhook_path_byte_identical():
    ...
    fired: list = []
    spy = _SpyServer()
    base, stop = _serve(spy, lambda tok: {"good-tok": "alice"}.get(tok),
                        webhook_nudge=lambda: fired.append(True))    # secret NOT set
    body = _RPC.encode("utf-8")
    hdrs = {"X-Hub-Signature-256": _sig("would-be-secret", body)}
    try:
        for token, want in ((None, 401), ("good-tok", 200)):
            s_hook, b_hook, h_hook = _request(f"{base}/census-webhook", body=body,
                                              token=token, headers=hdrs)
            s_mcp, b_mcp, h_mcp = _request(f"{base}/mcp", body=body,
                                           token=token, headers=hdrs)
            assert s_hook == want                          # pre-chunk semantics preserved
            assert (s_hook, b_hook) == (s_mcp, b_mcp)      # byte-identical to any other path
            assert h_hook.get("Content-Type") == h_mcp.get("Content-Type") == "application/json"
...
def test_run_http_dual_serves_webhook_on_tunnel_door_only(monkeypatch):
    ...
    runner = threading.Thread(
        target=lambda: H.run_http_dual(
            spy, host="127.0.0.1", loopback_port=0, tunnel_port=0,
            verify=lambda tok: {"good-tok": "alice"}.get(tok), lock=threading.Lock(),
            webhook_secret="s3cret", webhook_nudge=ev.set),
        daemon=True)
    runner.start()
    ...
    try:
        ...
        status, resp, _ = _request(f"http://127.0.0.1:{door['tunnel']}/census-webhook",
                                   body=body, headers=hdrs)
        assert status == 204 and resp == "" and ev.is_set()
        ev.clear()
        status, resp, _ = _request(f"http://127.0.0.1:{door['loopback']}/census-webhook",
                                   body=body, headers=hdrs)
