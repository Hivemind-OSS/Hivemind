...
def _root_cfg(tmp_path, **sync_kw) -> Config:
    return Config.load(db_path=":memory:", env={},
                       sync={"mirror_dir": str(tmp_path / "mirrors"), **sync_kw})


def _evidence(store) -> ChangeEvidenceService:
    return ChangeEvidenceService(reader=store, appender=store,
...
def test_sync_config_v3_surface():
    ...
    cfg = Config.load(db_path=":memory:",
                      env={"HIVE_SYNC__WEBHOOK_SECRET": "hook-credential"})
...
def test_start_sync_always_starts_thread_empty_registry_inert(store, tmp_path):
    ...
    thread = sync_mod.start_sync(_root_cfg(tmp_path), store, _evidence(store),
                                 threading.Lock())
    try:
...
def test_registered_repo_is_cloned_by_the_running_daemon(origin, store, tmp_path):
    ...
    thread = sync_mod.start_sync(_root_cfg(tmp_path, interval_s=5), store,
                                 _evidence(store), threading.Lock())
    try:
...
def test_start_sync_threads_lifecycle_through(store, tmp_path):
    ...
    marker = object()
    thread = sync_mod.start_sync(_root_cfg(tmp_path), store, _evidence(store),
                                 threading.Lock(), lifecycle=marker)
    try:
