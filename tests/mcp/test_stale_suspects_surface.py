...
def _seed_suspect(server, clock, text, *, anchor, payload=None, ts=None):
    """One servable (provisional, via hive_write) episode + one stale_suspect ledger row."""
    eid = write_text(server, text, anchors=[{"repo": "alpha", "anchor": anchor}])["id"]
...
def test_absent_unless_requested():
    server, clock = _rig()
    _seed_suspect(
        server, clock, "a suspected lesson", anchor="pkg/caller.py::caller_fn"
    )
...
def test_surfaces_newest_suspect_per_servable_episode():
    ...
    eid = _seed_suspect(
        ...
        payload=_suspect_payload(seed="old.py::S"),
        ts=100,
    )
    server.store.insert_audit(
        eid,
...
def test_bucket_is_anchor_ordered_and_lists_multiple_episodes():
    ...
    snap = content(tool_call(server, "hive_health", {"include_stale_suspects": True}))
    assert [r["episode_id"] for r in snap["stale_suspects"]] == [
        a,
        b,
    ]  # anchor-bucketed
...
def test_non_servable_episode_is_dropped_from_the_bucket():
    server, clock = _rig()
    servable = _seed_suspect(
        server, clock, "still servable", anchor="pkg/caller.py::caller_fn"
    )
    quarantined = content(
        tool_call(
            server,
            "hive_capture",
            {
                ...
                "anchors": [{"repo": "alpha", "anchor": "pkg/other.py::fn"}],
            },
        )
    )["id"]
    server.store.insert_audit(
        quarantined, EK_STALE_SUSPECT, "census", int(clock.now()), _suspect_payload()
    )
    snap = content(tool_call(server, "hive_health", {"include_stale_suspects": True}))
...
def test_never_suspected_store_returns_empty_list_when_requested():
    ...
    write_text(
        server,
        "an ordinary memory",
        anchors=[{"repo": "alpha", "anchor": "pkg/caller.py::caller_fn"}],
    )
    snap = content(tool_call(server, "hive_health", {"include_stale_suspects": True}))
