...
from tests.mcp._helpers import (
    build_real_server, content, is_error, register_repo, tool_call,
)
...
def test_recall_hits_carry_kind_and_anchors():
    ...
    content(tool_call(server, "hive_write",
                      {"text": "a labeled bug fact", "kind": "bug",
                       "anchors": [{"repo": "alpha",
...
def test_write_replaces_retires_target_only_when_qualified():
    ...
    server.store.insert_audit(a["id"], "outcome_verified_hurt", "census", 50,
                              _json.dumps({"stamp": {"head_sha": "c" * 40}}))
    c = content(tool_call(server, "hive_write", {"text": "v three", "replaces": a["id"]}))
...
def test_gap_report_carries_miss_scope():
    ...
    for agent in ("agent-A", "agent-B", "agent-A"):
        content(tool_call_as(server, agent, "hive_recall",
                             {"query": "how do i warm the alpha cache safely",
                              "repos": ["alpha"]}))
    snap = content(tool_call(server, "hive_health", {"include_gaps": True}))
    gaps = snap["gaps"]
