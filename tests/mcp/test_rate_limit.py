...
accrues exactly its floor of tokens), the `limit<=0` DISABLED sentinel (a caller-level
capability), per-key independence (AC4 — one flooding device never
...
def _limiter(
    limit: int = 3, window_s: float = 60.0, **kw
) -> tuple[TokenBucketLimiter, _Clock]:
    clock = _Clock()
