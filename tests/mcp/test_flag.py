...
def test_flag_idempotent_recorded_false_on_reflag():
    ...
    again = content(
        tool_call(server, "hive_flag", {"a": b, "b": a, "kind": "conflict"})
    )
...
def test_flag_unknown_episode_rejected_no_write():
    ...
    a, _b = _two(server)
    out = content(
        tool_call(server, "hive_flag", {"a": a, "b": 9999, "kind": "conflict"})
    )
...
def test_flag_supersedes_requires_winner():
    ...
    a, b = _two(server)
    out = content(
        tool_call(server, "hive_flag", {"a": a, "b": b, "kind": "supersedes"})
    )  # no winner
    assert out["status"] == "rejected"
...
def test_flag_secret_resolution_refused():
    ...
    out = content(
        tool_call(
            server,
            "hive_flag",
            {"a": a, "b": b, "kind": "conflict", "resolution": f"key {_AKIA}"},
        )
    )
...
def test_flag_schema_belt_rejects_missing_and_bad_kind():
    ...
    assert is_error(
        tool_call(server, "hive_flag", {"a": a, "b": b, "kind": "rumor"})
    )  # bad enum
