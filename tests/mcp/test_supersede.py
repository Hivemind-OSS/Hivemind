...
def _qualify(server, eid):
    ...
    server.store.insert_audit(eid, "outcome_verified_hurt", "census", 100,
...
def test_supersede_qualified_retires_loser_and_stamps_actor():
    ...
    actors = [r["actor"] for r in server.store.conn.execute(
...
def test_supersede_idempotent_rerun_no_duplicate_audit():
    ...
    loser, winner = _ids(server)
    _qualify(server, loser)
    assert content(tool_call(server, "hive_supersede",
                             {"loser": loser, "winner": winner}))["status"] == "superseded"
    n_audits = server.store.conn.execute(
        "SELECT COUNT(*) AS c FROM evidence_events WHERE episode_id=?",
        (loser,)).fetchone()["c"]
    again = content(tool_call(server, "hive_supersede",
                              {"loser": loser, "winner": winner}))
    assert again["status"] == "superseded"           # idempotent echo
    assert server.store.conn.execute(
        "SELECT COUNT(*) AS c FROM evidence_events WHERE episode_id=?",
        (loser,)).fetchone()["c"] == n_audits        # no duplicate audit rows
