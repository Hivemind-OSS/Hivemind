...
def _cluster_rig():
    ...
    ids = {
        "a": write_text(server, "ax=1 alpha: the retry helper doubles its backoff",
                        anchors=[{"repo": "alpha",
                                  "anchor": "hive/app/retry.py::backoff"}])["id"],
        "a2": write_text(server, "ax=2 alpha: the retry helper caps at five attempts",
                         anchors=[{"repo": "alpha",
                                   "anchor": "hive/domain/policy.py::cap"}])["id"],
        "b": write_text(server, "ax=3 beta: the retry helper jitters its delay",
                        repos=["beta"])["id"],
        "g": write_text(server, "ax=4 general: retries must be idempotent")["id"],
        "ab": write_text(server, "ax=5 shared: the retry helper is vendored in both",
                         repos=["alpha", "beta"])["id"],
    }
...
def test_every_hit_carries_repos_anchors_drift():
    server, _ = _drift_rig()
    a = write_text(server, "anchored fact one",
                   anchors=[{"repo": "alpha", "anchor": "pkg/one.py::fn"}])["id"]
...
def test_materialized_verdict_rides_the_hit():
    server, _ = _drift_rig()
    eid = write_text(server, "fact about the changed anchor",
                     anchors=[{"repo": "alpha", "anchor": "pkg/mod.py::fn"}])["id"]
    server.store.drift_put([("alpha", TIP, "pkg/mod.py::fn", "anchor_changed",
                             "{}", 5)])
...
def test_unmaterialized_anchor_reads_unverifiable_general_reads_na():
    server, _ = _drift_rig()
    cold = write_text(server, "fact whose tip was never materialized",
                      anchors=[{"repo": "alpha", "anchor": "pkg/cold.py::fn"}])["id"]
...
def test_name_at_branch_routes_ref_and_records_request():
    server, _ = _drift_rig()
    eid = write_text(server, "fact judged against a feature branch",
                     anchors=[{"repo": "alpha", "anchor": "pkg/feat.py::fn"}])["id"]
    server.store.drift_put([("alpha", TIP, "pkg/feat.py::fn", "fresh", "{}", 5)])
    env = _recall(server, "fact judged against a feature branch",
                  repos=["alpha@feature"])
    hit = next(h for h in env["reference_context"] if h["episode_id"] == eid)
    drift = hit["drift"]
    ...
    rows = [dict(r) for r in server.store.conn.execute(
        "SELECT repo, ref FROM ref_requests")]
...
def test_drift_reader_fault_degrades_not_breaks(monkeypatch):
    server, _ = _drift_rig()
    eid = write_text(server, "fact that must survive a drift-cache fault",
                     anchors=[{"repo": "alpha", "anchor": "pkg/faulty.py::fn"}])["id"]
