...
def test_acceptance_secret_write_refused_and_never_recallable(embedder_v1):
    ...
    secret_topic = (
        "rotate the leaked key AKIAIOSFODNN7EXAMPLE noted in the moonshot "
    ...
    assert (
        c.store.conn.execute(  # no blob either
            "SELECT 1 FROM blobs WHERE content_hash=?", (content_hash(secret_topic),)
        ).fetchone()
        is None
    )
...
def test_acceptance_reference_framing_and_trace_id(embedder_v1):
    ...
    resp = server.handle(
        MCPRequest(
            1,
            "tools/call",
            {"name": "hive_recall", "arguments": {"query": HIT_QUERIES[0][0]}},
        )
    )
    ...
    resp2 = server.handle(
        MCPRequest(
            2,
            "tools/call",
            {"name": "hive_recall", "arguments": {"query": MISS_QUERIES[0]}},
        )
    )
