...
def test_acceptance_recall_at_5_meets_floor(embedder_v1):
    ...
    assert mean_r5 >= 0.33, f"recall@5={mean_r5:.3f} below the §6.1#1 floor 0.33"


# ── §6.1 #2 — honest-abstention AUROC in band ─────────────────────────────────
def test_acceptance_abstention_auroc_in_band(embedder_v1):
    ...
    assert 0.70 <= auroc <= 1.0, f"abstention AUROC={auroc:.3f} outside the §6.1#2 band"


# ── §6.1 #3 — never-hallucinate / abstain-no-resurrect ────────────────────────
...
def test_acceptance_reference_framing_and_trace_id(embedder_v1):
    ...
    # an abstain STILL carries a trace_id (the §11 join key) and never fabricates hits
