...
def _duck_graphs(head_nx) -> object:
    ...
    return GraphPair(
        base=None, head=SimpleNamespace(nx=head_nx), base_stamp=None, head_stamp=None
    )
...
def test_additive_drift_seeds_are_filtered(head_nx) -> None:
    ...
    entries = propagation_block(
        (
            _duck_finding(drift="additive"),
            _duck_finding(drift="unchanged"),
            _duck_finding(drift=""),
        ),
...
def test_unknown_file_and_sourceless_nodes_are_skipped(head_nx) -> None:
    ...
    (entry,) = propagation_block(
        (
            _duck_finding(
                callers=("n_caller", "n_unknown", "n_file", "n_nofile"),
                tests=("n_test",),
            ),
        ),
        _duck_graphs(head_nx),
...
def test_neighbours_deduped_and_capped_sorted(head_nx) -> None:
    ...
    (entry,) = propagation_block(
        (
            _duck_finding(
                callers=tuple(ids), dependents=tuple(ids), tests=("n_3", "n_4")
            ),
        ),
        _duck_graphs(g),
        cap_neighbors=4,
...
def test_abstained_or_neighbourless_findings_produce_no_entry(head_nx) -> None:
    ...
    entries = propagation_block(
        (
            ...
            _duck_finding(callers=("n_unknown",)),
        ),  # nothing head-resolvable
        _duck_graphs(head_nx),
    )
    assert entries == []
