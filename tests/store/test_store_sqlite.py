...
def test_stage_never_indexes():
    ...
    blob = s.conn.execute(
        "SELECT text FROM blobs WHERE content_hash=?",
        (content_hash("db pool exhausted"),),
    ).fetchone()
    assert (
        blob is not None and blob["text"] == "db pool exhausted"
    )  # staged blob present
...
def test_stage_dedup_same_text():
    ...
    assert (
        a == b and d2 is True and s.counts()[1] == 1
    )  # one pending row ⇒ dedup, no 2nd insert
...
def test_complete_is_idempotent():
    ...
    assert (
        s.complete(eid, _unit(0, 1, 0, 0), expected_version=0, trust=PROVISIONAL)
        is True
    )  # no-op True
...
def test_cas_blocks_stale_complete():
    ...
    eid, _ = s.stage(text="x", weight=1.0, proposed_by="a")
    assert (
        s.complete(eid, _unit(1, 0, 0, 0), expected_version=99, trust=PROVISIONAL)
        is False
    )  # stale
...
def test_stage_dedup_preserves_existing_anchor_rows_unmerged():
    ...
    s = _store()
    a, _ = s.stage(
        text="same text", weight=1.0, proposed_by="a", anchors=[("alpha", "a.py::f")]
    )
    b, deduped = s.stage(
        text="same text",
        weight=1.0,
...
def test_get_episode_projects_repos_and_anchors():
    ...
    assert ep.anchors == (
        AnchorRef(repo="alpha", anchor="a.py::f"),
...
def test_rebuild_recovers_after_crash_between_commit_and_add():
    ...
    s.complete(
        e2, _unit(0, 1, 0, 0), expected_version=0, trust=ESTABLISHED
    )  # returns True (durable)
...
def test_stage_roundtrips_explicit_dont():
    s = _store()
    eid, _ = s.stage(
        text="never run rm -rf on the volume",
        weight=1.0,
        proposed_by="a",
        polarity="dont",
    )
    assert (
        s.get_episode(eid).polarity == "dont"
    )  # the prohibition survives the round-trip
