...
def _materialize(s: SqliteEpisodeStore, text: str, *, trust: str, vec_i: int = 0,
                 ts: int = 10, last_active=None, anchors=(), repos=()) -> int:
    eid, _ = s.stage(text=text, weight=1.0, proposed_by="w", ts=ts,
                     anchors=list(anchors), repos=list(repos))
    assert s.complete(eid, _unit(vec_i), expected_version=0, trust=trust,
...
def test_scopes_cover_exactly_the_servable_set():
    ...
    fresh = _materialize(s, "fresh provisional", trust=PROVISIONAL, vec_i=1,
                         last_active=NOW - 1)
    lapsed = _materialize(s, "lapsed provisional", trust=PROVISIONAL, vec_i=2,
                          last_active=NOW - P_TTL - 1)
...
def test_scope_is_the_union_of_anchor_repos_and_declared_scope():
    s = _store()
    eid = _materialize(s, "scoped fact", trust=ESTABLISHED,
...
def test_scope_only_membership_has_no_anchor_pairs():
    s = _store()
    eid = _materialize(s, "repo-scoped, no code binding", trust=ESTABLISHED,
                       repos=["alpha"])
    scope, pairs = s.servable_scopes(now=NOW, provisional_ttl_s=P_TTL)[eid]
