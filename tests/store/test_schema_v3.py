...
def test_v3_tables_present():
    ...
    assert {"episodes", "episode_anchors", "repos", "anchor_drift", "ref_requests",
            "blobs", "exposure", "meta", "recall_misses", "evidence_events",
            "conflict_flags", "ingested_ranges"} <= _tables(conn)
...
def test_dead_episode_columns_gone_from_ddl():
    ...
    ddl = conn.execute("SELECT sql FROM sqlite_master WHERE type='table' "
                       "AND name='episodes'").fetchone()["sql"]
...
def test_v3_lifecycle_columns_and_defaults():
    ...
    SqliteEpisodeStore(conn)
    ep_cols = _cols(conn)
    assert {"trust", "superseded_by", "last_active_ts", "polarity", "kind",
            "meta"} <= set(ep_cols)
...
def test_episode_anchors_shape():
    ...
    conn.execute("INSERT INTO episode_anchors(episode_id, repo, anchor) "
...
def test_anchor_drift_and_ref_requests_shape():
    ...
    conn.execute("INSERT INTO anchor_drift(repo, tip_sha, anchor, verdict, ts) "
