...
def _row(repo="alpha", tip=TIP0, anchor="a.py::f", verdict="fresh",
         detail="{}", ts=100) -> tuple:
...
def test_put_then_get_roundtrips_verdict_and_detail():
    s = _store()
    s.drift_put([_row(verdict="anchor_missing", detail='{"reason":"gone"}'),
                 _row(anchor="b.py::g", verdict="fresh")])
    got = s.drift_get("alpha", TIP0, ["a.py::f", "b.py::g"])
    assert got == {"a.py::f": ("anchor_missing", '{"reason":"gone"}'),
                   "b.py::g": ("fresh", "{}")}
...
def test_verdict_strings_ride_verbatim():
    ...
    s = _store()
    s.drift_put([_row(verdict="branch_scoped"), _row(anchor="x", verdict="unverifiable")])
...
def test_touch_then_requested_refs_roundtrip():
    ...
    assert s.requested_refs("alpha", since_ts=0) == ["develop", "feature"]  # ref-ordered
...
def test_touch_keeps_the_newest_request_stamp():
    ...
    row = s.conn.execute("SELECT last_requested_ts FROM ref_requests "
                         "WHERE repo='alpha' AND ref='feature'").fetchone()
