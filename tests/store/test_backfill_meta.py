...
def test_lacking_read_lists_each_binding_separately():
    s = _store()
    eid, _ = s.stage(text="two bindings", weight=1.0, proposed_by="w", ts=10,
...
def test_fill_anchor_fp_fills_empty_carrier_canonically():
    ...
    added = s.fill_anchor_fp(eid, REPO, ANCHOR,
...
def test_fill_anchor_fp_never_overwrites():
    ...
    assert s.fill_anchor_fp(eid, REPO, ANCHOR,
                            {"combdrift/fp": "combdrift-fp/1:again"}) == 0
...
def test_fill_anchor_fp_touches_only_the_addressed_row():
    ...
    episode_before = s.conn.execute(
        ...
        (eid,)).fetchone()
    assert s.fill_anchor_fp(eid, REPO, "a.py::f",
                            {"combdrift/fp": "combdrift-fp/1:abc"}) == 1
