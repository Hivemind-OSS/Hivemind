...
def stack(tmp_path):
    ...
    threads = [
        threading.Thread(target=h.serve_forever, daemon=True)
...
def test_tunnel_door_write_attributed_to_session_identity_then_recallable(stack):
    ...
    status, w = _call(
        ...
        {"text": text},
        token=token,
        headers={"X-Hive-Agent-Id": "alice-session-1"},
    )
...
def test_all_four_tools_answer_over_the_tunnel_door(stack):
    ...
    s_w, w = _call(
        tun, "hive_write", {"text": "a durable insight about retries"}, token=token
    )
    assert s_w == 200 and w["status"] == "approved"
    s_c, cap = _call(
        tun, "hive_capture", {"text": "a quarantined note about caching"}, token=token
    )
    assert s_c == 200 and cap["status"] == "quarantined"
    s_r, _r = _call(
        tun, "hive_recall", {"query": "a durable insight about retries"}, token=token
    )
    assert s_r == 200
...
def test_bad_token_writes_nothing_to_the_real_store(stack):
    ...
    code, _ = _call(
        tun, "hive_write", {"text": "should never land"}, token="hive_badtoken"
    )
...
def test_distinct_session_ids_are_distinct_identities(stack):
    ...
    _, w1 = _call(
        loop,
        "hive_write",
        {"text": "session-bound note one"},
        headers={"Mcp-Session-Id": "sess-A"},
    )
    _, w2 = _call(
        loop,
        "hive_write",
        {"text": "session-bound note two"},
