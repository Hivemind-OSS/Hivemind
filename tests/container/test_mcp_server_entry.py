...
from hive.tools import (
    entrypoint as E,
)  # ONLY for the marker key names this entry must NOT touch
...
def test_boot_order_then_serves():
    ...
    rc = M.main(
        argv=[],
        env=_OK_ENV,
        build_container_fn=_build_factory(container, captured),
...
def test_serve_does_not_stamp_readiness_markers():
    ...
    rc = M.main(
        argv=[],
        env=_OK_ENV,
        build_container_fn=_build_factory(container),
        serve=_serve_recorder(calls),
    )
    assert rc == M.EX_OK
...
def test_cli_args_take_precedence_over_env():
    ...
    M.main(
        argv=["--tenant", "fromcli", "--agent", "agentcli", "--db", "/tmp/x.db"],
...
def test_tenant_and_defaults_from_env():
    ...
    assert captured["tenant_id"] == "acme"
    assert (
        captured["agent_id"] == "default-agent"
    )  # default when neither arg nor env set it
    assert (
        captured["cfg"].db_path == "/data/shared.db"
    )  # default per-session store path
...
def test_empty_env_boot_does_not_pollute_stdout(capsys):
    M.main(
        argv=[],
        env={},
        build_container_fn=_build_factory(_RecordingContainer([])),
        serve=lambda s: None,
    )
...
def test_assemble_failure_exits_software():
    ...
    rc = M.main(
        argv=[],
        env=_OK_ENV,
        build_container_fn=build_container_fn,
        serve=_serve_recorder(calls),
    )
