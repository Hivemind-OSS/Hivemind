...
def test_create_prints_token_whose_hash_lands_in_table(db):
    out = io.StringIO()
    rc = authctl.main(
        ["create", "alice-laptop"], env={"HIVE_STORE__DB_PATH": db}, out=out
    )
...
def test_duplicate_label_is_nonzero(db):
    ...
    rc = authctl.main(
        ["create", "dup"], env={"HIVE_STORE__DB_PATH": db}, out=io.StringIO()
    )
    assert rc != authctl.EX_OK
...
def test_connect_fn_injection_uses_provided_conn():
    ...
    rc = authctl.main(
        ["create", "dave"],
        env={"HIVE_STORE__DB_PATH": "/ignored"},
        connect_fn=lambda _db: conn,
        out=out,
    )
    assert rc == authctl.EX_OK
...
def test_list_prints_provisioned_labels(db):
    ...
    authctl.main(
        ["create", "alice"], env={"HIVE_STORE__DB_PATH": db}, out=io.StringIO()
    )
    out = io.StringIO()
    rc = authctl.main(["list"], env={"HIVE_STORE__DB_PATH": db}, out=out)
    assert rc == authctl.EX_OK
